...
public class Chip {
	...
	//Chip-8 has a 4-kilobyte of memory, which is 8-bit memory
	private char[] memory;
	//Chip-8 has 16 registers called V
	private char[] V;
	//an address pointer, which is used to indicate
	//carry borrow and collision detection...
	//16-bits actually, but we'll use 12bits
	private char I;
	
	//the initial pointer where each program will start
	private char pc;
	...
	//They will tick down at 60 Hertz which equals,
	//each seconds will have 60 updates
	private int delay_timer;
	private int sound_timer;
	
	//hexadecimal input, which means 16 keys,
	//which will be stored in a char array 
	private byte[] keys;
	...
	public void init()
	{
		//a kilobyte is 1024, so 1024 * 4 = 4096
		...
		//the program will start at the position of hexadecimal 0x200
		pc = 0x200;
	...
	public void run()
	{
		...
		case 0x5000: { //SXYO Skips the next instruction if VX equals VY
			int x = (opcode & 0x0F00) >> 8;
		...
		case 0x8000: //Contains more data in last nibble
			...
			case 0x0006: { //8XY6: Shift VX right by one, VF is set to the least significant bit of VX
				...
				V[x] = (char)(V[x] >> 1);
				pc += 2;
				System.out.println("Shift V["+x+"] >> 1 and VF to least significant bit / LSB of VX");
				break;
			}
			
			case 0x0007: { //8XY7 Sets VX to VY - VX. VF is set to 0 when there's borrow, and 1 when there isn't.
				int x = (opcode & 0x0F00) >> 8;
				int y = (opcode & 0x00F0) >> 4;
			...
			case 0x000E: { //8XYE Shifts VX left by one. VF is set to the value of most significant bit of VX before the shift
				int x = (opcode & 0x0F00) >> 8;
				V[0xF] = (char)(V[x] & 0x80);
				V[x] = (char)(V[x] << 1);
				pc += 2;
				System.out.println("Shift V["+x+"] >> 1 and VF to most significant bit / MSB of VX");
				break;
			}
	...
	public void loadProgram(String file) {
		//IMPORTANT:
		//Use a DataInputStream to read every byte from input file.
		...
		try {
		    ...
		    {
		    	memory[0x200+offset] = (char)(stream.readByte()&0xFF);
		    	offset++;
		    }
